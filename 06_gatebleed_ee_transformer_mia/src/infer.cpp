...
int main(int argc, char* argv[]) {
	...
	// Init transformer
	Transformer transformer(2748, 256, 4, max_len, 96, 24, true);
	...
	std::string file = "the_prompt.txt";
	...
	// First, go through half the network
	vector<vector<float>> output = transformer.forward_with_early_exit(trunc_sequence, padding_mask, true);
	...
	// Check the entropy
	double entropy = ((probabilities[0]*log2(probabilities[0])) + (probabilities[1]*log2(probabilities[1]))) * -1.0;
	std::cerr << "Entropy: " << entropy << std::endl;

	// j-statistic calculated in build/*.ipynb
	if (entropy >= 0.255241) {
		// Forward through second half of layer (technically just running first half again, but equivalent amount of computation)
		vector<vector<float>> output_2 = transformer.forward_with_early_exit(trunc_sequence, padding_mask, true);
		std::vector<float> pooled_output_2 = mean_pooling(output);
		vector<float> logits_2 = linear_layer(pooled_output, final_weights, final_bias);
		vector<float> probabilities_2 = softmax(logits);		
	}
	else {
		return 0;
	}
